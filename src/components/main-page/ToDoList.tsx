 ...
 */
export const ToDoListItems = ({ ToDo, defaultChecked = false, onToggle }: ToDoListItemsProps) => {
    ...
    if (!ToDo || ToDo.trim().length === 0) {
        return (
            <img
                src={NullToDo}
                alt="todo-null" //테스트용 고정 alt
                data-testid="null-image"// 테스트용 id
            />
        );
    }
    const currentSrc = checked ? Checked : NoneChecked; // 체크 상태
    /** 채크박스 토글 */
    const handleToggle = () => {
        const next = !checked;
        setChecked(next);
